	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case NO_WIFI:
				...
				failAlert.setTitle("???????????").setIcon(R.drawable.cloud_fail).setMessage("????????????υτ??????????");
				failAlert.create().show();
				break;
			case CMD_TIMEOUT:

				failAlert.setTitle("???????????υτ").setIcon(R.drawable.cloud_fail).setMessage("?????????????υτ");
				failAlert.create().show();
				break;
			case USER_EXISTED:
